...
class Queryable(Source, metaclass=abc.ABCMeta):
    ...
    @classmethod
    def _expression(cls, source: frame.Queryable,
    ...
    @classmethod
    def _subcondition(cls, source: frame.Queryable,
                      handler: typing.Callable[[frame.Queryable, series.Expression], frame.Query],
                      target: typing.Callable[[frame.Query], series.Expression]):
        """Common subcondition (cumulative condition) testing routine (for where/having).
        """
        cls._condition(source, handler, target)
        assert target(handler(handler(source, source.score > 2), source.level < 4)) == \
               function.And(function.LessThan(source.level, series.Literal(4)),
                            function.GreaterThan(source.score, series.Literal(2)))
